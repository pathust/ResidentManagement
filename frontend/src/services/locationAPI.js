...
export const statsAPI = {
  ...
  // Thống kê nhân khẩu theo phường
  getWardPersons: async (wardId) => {
    try {
      const { data } = await apiClient.get(`/api/stats/wards/${wardId}/persons`);
      return data;
    } catch (error) {
      throw new Error(error.response?.data?.message || 'Không thể tải thống kê phường');
    }
  },
  
  // Thống kê hộ khẩu theo phường
  getWardHouseholds: async (wardId) => {
    try {
      const { data } = await apiClient.get(`/api/stats/wards/${wardId}/households`);
      return data;
    } catch (error) {
      throw new Error(error.response?.data?.message || 'Không thể tải thống kê hộ khẩu phường');
    }
  }
};
...
export async function POST(req) {
    ...
    try {
        const response = await fetch(`${SERVER_URL}/api/households/splits`, {
            method: "POST",
            headers: {
        ...
        if (!response.ok) {
            ...
            return NextResponse.json({ error: errorData.detail || "Failed to split household" }, { status: response.status });
        }

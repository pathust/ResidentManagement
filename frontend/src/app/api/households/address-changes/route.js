...
export async function POST(req) {
    ...
    try {
        const response = await fetch(`${SERVER_URL}/api/households/address-changes`, {
            method: "POST",
            headers: {
        ...
        if (!response.ok) {
            ...
            return NextResponse.json({ error: errorData.detail || "Failed to change household's address" }, { status: response.status });
        }
